...
#	usuario = User.objects.get(pk=int(id))
	...
	usuario_usuario = Usuario.objects.get(cedula=int(usuario.username))
	...
	d = {}
	d['usuario']=usuario
	d['usuario_usuario'] = usuario_usuario
	#for i in d:
	#	print i,d[i]
...
def nuevo_usuario(request):
	...
	if request.method == 'POST':
		formulario = crear_usuario(request.POST)
		if formulario.is_valid():
			...
			username_usuario = formulario.cleaned_data['username_usuario']
			usuario=User.objects.get(username=username_usuario)

			#usuario=User.objects.get(username=formulario.cleaned_data['username_usuario'])
			usuario.password = make_password(formulario.cleaned_data['clave_usuario'])
			
			usuario.first_name = formulario.cleaned_data['nombre_usuario']
			usuario.last_name = formulario.cleaned_data['apellido_usuario']
			usuario.email = formulario.cleaned_data['correo_usuario']
			usuario.is_active = True

			usuario.save()	
			return render_to_response('nuevo_usuario.html',{'aviso':"Se han Guardado los Datos Satisfactoriamente"}, context_instance=RequestContext(request))
    
	else:
		d={}
		#d['formulario'] = crear_usuario()
		d['formulario_cedula'] =  form_consultar_cedula(initial={'nombre_usuario':'123'})

	return render_to_response('nuevo_usuario.html',d, context_instance=RequestContext(request))

def consultar_cedula(request):
	d={}
	print "consultar_cedula"
	if request.method == 'POST':
		try:
			formulario = form_consultar_cedula(request.POST)
			print formulario
			#print " 777777777777777    %s    666666666  " %formulario.cleaned_data
			u_usuario = formulario.cleaned_data['cedula_trabajador']
			usuario=User.objects.get(username=u_usuario)
			print " >>>>>>>>> /// %s    /////// " %usuario
			valores={'username_usuario': usuario.username, 'nombre_usuario':usuario.first_name , 'apellido_usuario': usuario.last_name }
			print valores
			d['formulario']=crear_usuario(initial=valores)
		except Exception,e:
			d['error'] = " No existe Trabajador"
	return render_to_response('nuevo_usuario.html',d, context_instance=RequestContext(request))




def consultar_trabajador(request):
	print "hola"
		...
		if formulario.is_valid:
			...
			if acceso is not None:
				...
				if acceso.is_active:
					...
					if not is_staff:
						d['usuario_usuario']= Usuario.objects.get(cedula=user_user)
...
#			historial_solicitudes
		...
		print "\n\n\n ['inicio_relacion_laboral'] %s \n\n\n\n"%result[0]['inicio_relacion_laboral']
		#fecha=str(tipo['anno'])+"-"+str(tipo['mes'])+"-"+"01"
		fecha=str(tipo['anno']+"-"+tipo['mes']+"-"+"01")
		#fecha="2015-12-11"
		print "\n\n\n fecha  >>>>>>>>>>> %s \n\n\n\n"%fecha


		fecha_solicitud = datetime.strptime(fecha, "%Y-%m-%d")
		#fecha_inicio_laboral = datetime.strptime(result[0]['inicio_relacion_laboral'] , "%Y-%m-%d") 
 		fecha_inicio_laboral = datetime.strptime(result[0]['inicio_relacion_laboral'] , "%Y-%m-%d") 
		
		print "\n\n\n fecha solicitar  %s   >>>><<<<  fecha inicio laboral %s \n\n\n\n"%(fecha_solicitud,fecha_inicio_laboral)

		if( fecha_inicio_laboral >	 fecha_solicitud):
			d['aviso']="El inicio de su relacion Laboral no esta comprendida en el periodo seleccionado"
		else:
...
def validar_solicitud_qr(request,parametros):

	print "validar_solicitud CON PARAMETROS"
	print parametros
	error=False
	d={}
	if request.method == "GET":
		...
		try:
			if form.is_valid():
				...
				d['validado']=consultar_codigo(parametros)
				#d['validado']=True
				print " \n \n \n \n \n  validado ======================  \n %s" %d
				d['error']=False
		except Exception,e:
...
def ivss(request):
	...
	#print " ... . . . .. %s  . . . . . . " %request.user.get_username()
	
	usuario = Usuario.objects.get(cedula=request.user.get_username())

	d['usuario']=usuario 


	return render_to_response('ivss.html',d,context_instance=RequestContext(request))
